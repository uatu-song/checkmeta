...
class MetaLeagueSimulator:
    ...
    def apply_end_of_round_effects(self, characters, context, show_details=True):
        ...
        self.update_team_momentum(context)
        ...
        for character in characters:
            ...
            trait_heal_amount = 0
            healing_traits = self.check_trait_activation(character, "end_of_turn", context)
            for trait_activation in healing_traits:
                trait_id = trait_activation["trait_id"]
                if trait_id in self.traits:
                    ...
                    if trait.get("formula_key") == "heal":
                        ...
                        context["trait_logs"].append({
                            ...
                            "trait": trait["name"],
                            "effect": f"+{trait_heal_amount} healing",
                            "value": trait_heal_amount
                        })
                        
                        # Update combat stats
                        character["combat_stats"]["special_ability_uses"] += 1
            ...
            if character.get("HP", 100) < 100:
                ...
                # Log healing
                if trait_heal_amount > 0 and show_details:
                    print(f"  {character['name']} healed for {heal_amount} HP (trait: {trait_heal_amount})")
            ...
            # Apply momentum bonus to regeneration
            momentum_bonus = 0
            if character.get("team") == "A" and "team_a_momentum" in context:
                if context["team_a_momentum"]["state"] == "crash":
                    momentum_bonus = 3  # Bonus regeneration for teams in comeback mode
            elif character.get("team") == "B" and "team_b_momentum" in context:
                if context["team_b_momentum"]["state"] == "crash":
                    momentum_bonus = 3  # Bonus regeneration for teams in comeback mode
            
            regen_rate = base_regen + wil_regen + momentum_bonus
            ...
            if character.get("is_ko", False):
                ...
                if stamina > 20:  # Lowered threshold from 30
                    ...
                    # Increase recovery chance for teams in crash momentum
                    if character.get("team") == "A" and "team_a_momentum" in context:
                        if context["team_a_momentum"]["state"] == "crash":
                            recovery_chance *= 1.5  # 50% higher chance to recover when in comeback mode
                    elif character.get("team") == "B" and "team_b_momentum" in context:
                        if context["team_b_momentum"]["state"] == "crash":
                            recovery_chance *= 1.5  # 50% higher chance to recover when in comeback mode
