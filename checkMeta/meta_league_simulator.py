...
class MetaLeagueSimulator:
    ...
    def simulate_match(self, team_a, team_b, show_details=True):
        ...
        # Determine team colors based on day number
        if self.current_day % 2 == 1:
            # Odd days: Team A is White, Team B is Black
            team_colors = {unit["id"]: chess.WHITE for unit in team_a}
            team_colors.update({unit["id"]: chess.BLACK for unit in team_b})
        else:
            # Even days: Team A is Black, Team B is White
            team_colors = {unit["id"]: chess.BLACK for unit in team_a}
            team_colors.update({unit["id"]: chess.WHITE for unit in team_b})

        # Initialize boards
        for unit in team_a + team_b:
            color = team_colors[unit["id"]]
            board = chess.Board()
            if color == chess.BLACK:
                board.turn = chess.BLACK  # Set black to move first
            role = unit["role"]
