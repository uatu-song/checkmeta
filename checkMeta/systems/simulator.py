...
class MetaLeagueSimulator:
    ...
    def simulate_match(self, team_a: List[Character], team_b: List[Character], show_details: bool = True) -> Dict[str, Any]:
        ...
        # Simulate moves
        for move_num
        # Continue from systems/simulator.py
        # Simulate moves
    ...
    def _update_character_metrics(self, character: Character, material_change: int, show_details: bool = False) -> None:
        ...
        if material_change < 0:
            # Reduce damage scaling from material loss
        ...
        # Material gain = damage dealt to opponent
            ...
            # Reduce damage scaling for stats
        ...
        # Reduce stamina cost for moving
        stamina_cost = 0.5
        
        # Apply WIL bonus to reduce stamina cost
        wil_bonus = (character.get_attribute('WIL') - 5) * 0.08  # 8% reduction per point above 5
        ...
        for character in characters:
            ...
            for effect in trait_effects:
                if effect.get("effect") == "healing":
                    trait_heal_amount += effect.get("value", 0) * 3  # Tripled healing effect
                    match_result.add_trait_log({
                        "round": match_result.round,
                        "character": character.name,
                        "trait": effect.get("trait_name"),
                        "effect": f"+{effect.get('value') * 3} healing"
                    })
            
            # Apply recovery
            recovery_result = character.recover(base_hp_regen, 5, trait_heal_amount)
