...
class MetaLeagueSimulator:
    ...
    def activate_advanced_features(self):
        try:
            ...
            return {"traits": False, "xp": False, "stockfish": False, "morale": False}
    ...
    def simulate_match(self, team_a, team_b, show_details=True):
        ...
        # --- Process post-match XP if available ---
        if self.advanced_features_enabled and hasattr(self, 'xp_system'):
            for character in team_a + team_b:
                xp_earned = self.xp_system.calculate_match_xp(character, match_context)
                level_result = self.xp_system.apply_xp_and_level(character, xp_earned)

        if show_details and level_result["level_ups"] > 0:
            print(f"{character['name']} leveled up to level {level_result['new_level']}!")
